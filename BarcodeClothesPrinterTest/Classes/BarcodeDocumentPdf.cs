...
{
    public class BarcodeDocumentPdf
    {
        ...
        public BarcodeDocumentPdf(string _pdfPath)
        {
        ...
        public BarcodeDocumentPdf(string _pdfPath, Rectangle pageSize)
        {
            b = new BarcodeLib.Barcode();
        ...
        /// <returns></returns>
        iTextSharp.text.Image CreateImage(string code,
            System.Drawing.PointF point, int width, int height)
        {

            var barcodeImage = iTextSharp.text.Image.GetInstance(GetBarcodeImage(code, width, height));
            barcodeImage.SetAbsolutePosition(point.X, doc.PageSize.Height - point.Y);
            return barcodeImage;
        }
        ...
        public void CreateA7BarcodeDocument()
        {
            ...
            for (int i = 0; i < Barcodes.Count; i++)
            {
                if (i != 0)
                    doc.NewPage();
                CreateProductInfo(Barcodes[i].ImageCode, Barcodes[i].BarcodeLabel,
                    Barcodes[i].ProductTypeRank);
           ...
           string typeAndRank)
        {
            ...
            doc.Add(CreateImage(ImageCode,
               new System.Drawing.PointF((doc.PageSize.Height / 2) - 30,
               (doc.PageSize.Width / 2) - 50), 160, 30));
            CreateChunck(typeAndRank, writer,
                new iTextSharp.text.Rectangle(100, 90, 200, 170));
            CreateChunck(barcodeText, writer,
                new iTextSharp.text.Rectangle(100, 30, 190, 120));
       ...
       string typeAndRank)
        ...
        iTextSharp.text.Image GetBarcodeImage(string barcode_string, int width, int height)
        {

            System.Drawing.Image myimg =
              b.Encode(BarcodeLib.TYPE.CODE128A, barcode_string.Trim()
                    , System.Drawing.Color.Black,
                   System.Drawing.Color.White, width, height);
